...
def move_to_problem(imap_uid):
    ...
    try: conn.copy(imap_uid,"PROBLEM"); conn.store(imap_uid,'+FLAGS','\\Deleted'); conn.expunge()
    finally: conn.logout()
...
def process_retries():
    ...
    for row in rows:
        ...
        try: send_notification(bounced_email,subject,reason,notify_cc); c.execute("DELETE FROM retry_queue WHERE id=?",(id,))
        except Exception:
            if retries+1>=MAX_RETRIES: move_to_problem(imap_uid); c.execute("DELETE FROM retry_queue WHERE id=?",(id,))
            else: c.execute("UPDATE retry_queue SET retries=retries+1,last_attempt=? WHERE id=?",(datetime.utcnow().isoformat(),id))
    conn.commit(); conn.close()
if __name__=="__main__": process_retries()
